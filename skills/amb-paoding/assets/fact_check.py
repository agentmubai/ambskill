# -*- coding: utf-8 -*-
"""事实核对：技能交付前跑一遍——成品里出现的数字、年限、价格、人数，必须来自「事实清单」或写成〔槽位〕，否则报出来。
拆的是对方的作品，这一条尤其要紧：样例里的数字、人名、地名不能顺手写进使用者的稿。
生产成品时，本文件复制进每个类型技能的 scripts/fact_check.py（技能目录单独拷走也能跑）；成品根目录 scripts/ 另放一份作开放资产。
用法：python3 <技能目录>/scripts/fact_check.py <回答.md> [<技能目录>]     退出码 0 = 无可疑；1 = 有可疑事实；2 = 没找到事实清单 / 用法错
给了技能目录时，出现在其 SKILL.md / references 里的数字视为"结构数字"（前三秒、三段式这类骨架参数），单独列出不算可疑。
