...
| S5 蒸馏 | 每任务知识包 + 六层 + 卡 | 每任务：`distill --step pack` → 派 → `caselib` → `screen` → `distill --step layers` → 派 → `check-layers` → `distill --step cards` → 派；最后 `index` | 每任务 3 | 四件齐；每任务 check-layers 通过；index 在 | 05-distill.md |
